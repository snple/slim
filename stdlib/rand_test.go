...
func TestRand(t *testing.T) {
	...
	module(t, "rand").call("seed", seed).expect(slim.UndefinedValue)
	...
	buf1 := make([]byte, 10)
	buf2 := &slim.Bytes{Value: make([]byte, 10)}
	n, _ := r.Read(buf1)
	...
	randObj.call("seed", seed).expect(slim.UndefinedValue)
	...
	buf1 = make([]byte, 12)
	buf2 = &slim.Bytes{Value: make([]byte, 12)}
	n, _ = r.Read(buf1)
